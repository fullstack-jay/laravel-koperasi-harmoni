...
use Modules\V1\Supplier\Requests\UpdateHargaLoadDataRequest;
use Modules\V1\Supplier\Resources\POUpdateHargaResource;
use Shared\Helpers\ResponseHelper;

final class SupplierUpdateHargaController
{
    /**
     * @OA\Post(
     *     path="/Suppliers/UpdateHarga/LoadData",
     *     summary="Load POs for price update",
     *     description="Get list of purchase orders that are ready for price update by supplier. For supplier role: returns their own POs. For super_admin role: must provide supplierId parameter.",
     *     tags={"Suppliers"},
     *
     *     @OA\RequestBody(
     *         required=true,
     ...
     *                 @OA\Property(property="poNumber", type="string", example="PO-20260301-0001"),
     ...
     *                     @OA\Property(property="category", type="string", example="SEMBAKO", description="Category from stock_items table (e.g., KMN, KEMASAN, SEMBAKO)"),
     ...
     * )
     */
    public function loadData(Request $request, UpdateHargaLoadDataRequest $loadDataRequest)
    {
        try {
            $user = $request->user();
            ...
            $query = PurchaseOrder::with(['items.stockItem', 'supplier', 'createdBy'])
                ...
                ->where('status', POStatusEnum::MENUNGGU_PERSETUJUAN_SUPPLIER);
            ...
            return ResponseHelper::paginate(
                POUpdateHargaResource::collection($po->items()),
                $po->total(),
                $po->perPage(),
                $po->currentPage()
            );
